...
export default class AdminAttributeNamesPage extends React.Component {
    ...
    getPage(pageNumber) {
        axios.get(constants.SERVER_ADDRESS + "/products/attribute-name/?page=" + pageNumber).then(response => {
            this.setState({
                ...this.state,
                ...
                <table className="table">
                    ...
                    { this.state.attributeNames.map((attributeName, idx) => (
                        <tr key={idx}>
                            ...
                            <td><a href={"/admin/attribute-name/update/" + attributeName.id} className="btn btn-primary">Редактировать</a><div className="btn btn-danger" onClick={() => this.handleDelete(attributeName.id, attributeName.title)}>Удалить</div></td>
                        </tr>
                    ))}
                    </tbody>
                </table>
